...
CITYPLAN.Attachment = function (earth) {
    ...
    var avwinpath= getClientPath() + "avwin\\avwin.exe";   //cy：注册的客户端下放入avwin文件夹
...
//    };
     ...
     //cy:64位
    function clickNode(event, treeId, treeNode) {
        // var returnStatus = earth.UserDocument.SaveFile("http://192.168.10.162/se_city_plan?type=blob&operation=get&tablename=CPATTACHMENT&filed=OBJ&id=ed889159-b00f-4449-a729-56ac74ec18f4&blobtype=jpg", "greatwork.jpg");
        // alert(returnStatus);
        // return;
        var url;
        var type = treeNode.attachmentType;
...
//                exec('file:///C:/av/avwin/avwin.exe', filePath);
            ...
            //     }).panel({width:earthWidth, height:earthHeight})
            //         .panel("move", {top:113+"px", left:250 + earthWidth + "px"});
            // url += ("&width=" + earthWidth + "&height=" + earthHeight);
            // top.$("#ifScreen2D").attr("src", "html/investigate/planCheckImage.html?url=" + url);
        }
    };
        ...
        //从当前选中的树
            ...
            if(res && res[0]){
                ...
                if(picID){
                    ...
                    //调用exe打开文件
                    var returnStatus = earth.UserDocument.SaveFile(url, picID + "." + type);
                    if(returnStatus){
                        var filePath = earth.RootPath + "\\temp\\" + picID + "." + type;
                        // exec('file:///C:/av/avwin/avwin.exe', filePath);
                        exec(avwinpath, filePath);
                    }
                }
