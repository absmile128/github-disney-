...
CITYPLAN.ProjImport = function (earth, dataProcess, generateEdit) {
    ...
    */
    ...
    var AddLayer = function (projectGuid, groupID, layerName, type, group, meshDic, layerIDs) {
       ...
       earth.DatabaseManager.AddLayerInDatabaseIncludeGroupID(param, -2);
...
//                    "<TYPE>4</TYPE><SJDW />" +
    ...
    */
    var ImportBuildingsInfo = function (BuildingPath, projectGuid, planID, totalFile) {
        ...
        if(buildNum==undefined){buildNum=1;}

        //遍历每一个建筑节点
        for (var i = 0; i < buildNum; i++) {
            var result = false;
            var buildNode=null;
            if(buildNum==1){ buildNode = xmlJSON.BUILDING;} //只有一个建筑模型
            else
            {  buildNode = xmlJSON.BUILDING[i];}
    ...
    var _deleteModelPlan = function (planId) {
        ...
        if (res.length > 0) {
            ...
            $.each(layerIdArr, function (i, id) {
                earth.DatabaseManager.DeleteLayerInDatabase(id);
            });
        }
        // 删除属性数据
    ...
    */
    ...
    var _deleteSubjectByProjectId = function (projectId) {
        ...
        var xml = "";
        if (parcelLayerId) {
            earth.DatabaseManager.DeleteLayerInDatabase(parcelLayerId);
            xml = "<PARCEL><PROJECTID> ='" + projectId + "' </PROJECTID></PARCEL>";
            dbUtil(CITYPLAN_config.service.remove, xml);
        }
        if (roadlineLayerId) {
            earth.DatabaseManager.DeleteLayerInDatabase(roadlineLayerId);
            ...
            for (var i = 0; i < layer.GetFeatureCount(); i++) {
                ...
                if (geoType == 3 || geoType == 403) {   // SEWkbGeometryType.SEWkbPolygon
                    ...
                    if (vects.Count > 0) {
                     ...
                     earth.DatabaseManager.AddElementListInLayer(layerId, infolist);
    ...
    */
    var _savePolygonObject = function (param) {
        ...
        if (infolist.Count > 0) {
            ...
            earth.DatabaseManager.AddElementListInLayer(param.guid, infolist);
        }
    };
            ...
            for (var i = 0; i < nodeNum; i++) {
                ...
                if (nodeLen > 0) {
                    ...
                    for (var k = 0; k < nodeLen; k++) {
                        ...
                        srcLct.X = obj.Pivot.X + offset.X;
                        srcLct.Z = obj.Pivot.Z + offset.Y;
                        srcLct.Y = obj.Pivot.Y;
    ...
    */
    var _addRecordInLayer = function (node, param, offset, dic) {
        ...
        if (meshid >= 0) {
            ...
            earth.DatabaseManager.AddRecordInLayer(param.guid, databasedata);
        }
    };
