 ...
 * Date: 12-12-13
 * Time: 上午9:40
 * Desc: 依赖jquery、jquery.xml2json库，CITYPLAN_config.js配置文件
 */
if( !STAMP ){
    var STAMP = {};
}
...
STAMP.TrackManager = function (earth){
        ...
        earth.Event.OnDynamicListLoaded = function (list){
            ...
            if(list.Count == 0){
                ...
                if(selectObj.val() == null){
                    selectObj.customAttr("disabled", "disabled");
                }
            }
        };
    ...
    var _appendStationPass = function (res, route){
        ...
        for(var i=0; i<route.GetChildCount(); i++){
            ...
            res.push({ id: pass.Guid, name: pass.Name, pId: route.Guid });
        }
    };
    /**
     * 获取track中所有对象的内容
    ...
    var _getStations = function (track){
        ...
        while(i < track.GetChildCount()){
            ...
            if(station.Name=="pass"){
                result.push({ id: station.Guid, name: station.Name, pId: track.Guid,open:true});
            } else{
                result.push({ id: station.Guid, name: station.Name, pId: track.Guid});
            }
    ...
    var _appendLookat = function (track, xml){
        ...
        var lookat = earth.Factory.CreateStationLookat ( $lookat.customAttr("id"), $lookat.customAttr("name") );
        lookat.Longitude = $lookat.customAttr("Longitude");
        lookat.Latitude = $lookat.customAttr("Latitude");
        lookat.Altitude = $lookat.customAttr("Altitude");
        lookat.Heading = $lookat.customAttr("Heading");
        lookat.Tilt = $lookat.customAttr("Tilt");
        lookat.Roll = $lookat.customAttr("Roll");
        lookat.range = $lookat.customAttr("Range");
        //lookat.time = $lookat.customAttr("Time");
        lookat.StopTime = $lookat.customAttr("StopTime");
    ...
    var _appendSurround = function (track, xml){
        ...
        var surround = earth.Factory.CreateStationSurround ( $surround.customAttr("id"), $surround.customAttr("name") );
        surround.Longitude = $surround.customAttr("Longitude");
        surround.Latitude = $surround.customAttr("Latitude");
        surround.Altitude = $surround.customAttr("Altitude");
        surround.FlyHeight = $surround.customAttr("FlyHeight");
        surround.Speed = $surround.customAttr("Speed");
        surround.Radius = $surround.customAttr("Radius");
        surround.NumberOfCycle = $surround.customAttr("NumberOfCycle");
    ...
    var _createTrack = function (id, name){
        //var track = earth.TrackControl.GetTrack(id);
        //if(track){
        // return;
        // }else{
        ...
        $($.text2xml(xml)).find("Track").first().children().each(function (){
            switch(this.tagName){
                case "Route":
                    _appendRoute(track, xml);
                    break;
                case "Lookat":
                    _appendLookat(track, xml);
                    break;
                case "Surround":
                    _appendSurround(track, xml);
                    break;
            }
        });
