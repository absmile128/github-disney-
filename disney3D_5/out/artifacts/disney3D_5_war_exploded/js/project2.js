...
//    };
    ...
    var internalFun = function(){
        // top.$("body").append('<div id="impDialog" class="easyui-dialog" title="导入项目"  data-options=\"iconCls:'icon-save'\" style="width:200px;height:150px;padding:10px;" ><label id="impDialogTxt">正在导入中......</label></div>');
            ...
            for (var i = 0; i < nodeNum; i++){
                ...
                if(nodeLen > 0){
                    ...
                    for(var k = 0; k < nodeLen; k++){
                       ...
                       //cy改 var result = generateEditDll.run_single_ref_no_thread(param.guid, server, filepath, modelName, param.SpatialRef, logpath, savemethod);
        ...
        if (isImportFinished){
            ...
            //         parent.getEditLayerListLoaded();
                ...
                setTimeout(function(){
                    parent.getEditLayerListLoaded();
                    //parent.savePosition();
                    //$("#ifEarth",top.document).attr("src", "3DMain.html");
                }, 300);
            }, 200);
        }
    };
