...
//    }
    ...
    //cy:对gis数据分页进行查询
    var  gisSearchBypage  = function(layerguid,pcStr,scStr,pageNo,pageSize) {

        var data=null;
        var pagging = "&pg=" + (pageNo - 1) + "," + pageSize;
        var url = config.server.ip + "/geoserver?service=" + layerguid + "&qt=16" + scStr + pcStr+pagging  ;
        ...
        var data2={
            TotalCount:resultObj.Result.num,
            List:data
        }
...
//    };
    ...
    var gisSearch = function(layerguid,str) {
      ...
      var url = config.server.ip + "/geoserver?service=" + layerguid + "&qt=17" +str+ "&pg=0,100000";
        // var url = config.server.ip + "/geoserver?service=" + layerguid + "&qt=17" +str+ "&pg=0,"+pageRecordCount;
        var result = loadXMLStr(url );
        var resultObj = $.xml2json(result);
        if (resultObj && resultObj.Result && resultObj.Result.num != 0) {
            var arr = [];
...
//    };
  ...
  //  search.keyWordSearch = keyWordSearch;
   ...
   // search.getGeoDetail  =getGeoDetail;//cy加2017
    ...
    search.gisSearchBypage = gisSearchBypage//cy:对gis 分页查询
