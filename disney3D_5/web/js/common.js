 ...
 * @returns 子窗口传给父窗口的参数
 */
function openDialog(url,params,width,height,resizablebool){
    ...
    if(is_ie_6){
        height = height + 50;
        value = window.showModalDialog(url,params,"menubar:no;dialogWidth:"+width+"px;status:no;title:no;help:no;resizable:"+resizable+";scroll:yes;location:no;toolbar:no;dialogHeight:"+height+"px");
    }else{
        value = window.showModalDialog(url,params,"menubar:no;dialogWidth:"+width+"px;status:no;title:no;help:no;resizable:"+resizable+";scroll:yes;location:no;toolbar:no;dialogHeight:"+height+"px");
    }
    return value;
...
//cy提示框信息
function tipdivshow(type,isblock) {
	if (type == "search") {
		if (isblock) {
			$("#nodatatip_div").css("display", "none");
			$("#searchtip_div").css("display", "block");
		}
		else {
			$("#nodatatip_div").css("display", "none");
			$("#searchtip_div").css("display", "none");
		}
	} else if (type == "nodata") {
		if (isblock) {
			$("#nodatatip_div").css("display", "block");
			$("#searchtip_div").css("display", "none");
		}
		else {
			$("#nodatatip_div").css("display", "none");
